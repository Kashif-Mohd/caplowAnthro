...
{
    public partial class showform2 : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_form2 where dssid LIKE '%" + txtdssid.Text + "%' and (str_to_date(q01, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) order by mother_id,child_id,followup_num", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_form2 WHERE dssid LIKE '%" + txtdssid.Text + "%'  order by mother_id,child_id,followup_num", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=Form-2 (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
